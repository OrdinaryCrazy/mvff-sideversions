...
def propagate_key_lines(key_frame_predictions, mapping, num_image_in_group):
    ...
    key_frame_global_id = int(key_frame_predictions[-1][0])
    ...
    for group_idx in range(1,num_image_in_group + 1):
        ...
        try:
            if key_frame_mv is None:
                mv_full = mv_full
            else:
                mv_full = mv_full - key_frame_mv
        except:
            print key_frame_global_id
            print group_idx
            print file_path
            print mapping[str(key_frame_global_id)]
    ...
    for idx in range(10000):
        if startingIdx + idx < 19614314:
            ...
            if group_line_local_id < key_frame_local_id:
                ...
                if num_image_in_group == (STEP // FIC) - 1:
                    return (STEP // FIC) - 1
        else:
            return num_image_in_group
...
def process():
    ...
    for line_idx in range(len(lines)):
        ...
        if frame_local_id % (STEP // FIC) == 0:
            ...
            #     print line_split[0]
            #     print 'key_frame_mv not found'
            #     print file_path + '.pkl'
            #     key_frame_mv = None
            #     # input()
            # else: 
            #     key_frame_mv = load_mv(file_path)

            key_frame_mv = load_mv(file_path)

        else:
            # Non-key frame.
            if last_line_local_id % (STEP // FIC) == 0:
                ...
                # num_image_in_group = get_num_image_in_group(lines, last_line_local_id, mapping)
