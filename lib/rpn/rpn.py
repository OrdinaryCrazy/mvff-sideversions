...
from utils.image import *#get_image, get_image_mv, get_pair_image, get_pair_image_mv, tensor_vstack
...
def get_rpn_testbatch_mv(roidb, cfg):
    """
    return a dict of testbatch
    :param roidb: ['image', 'flipped']
    :return: data, label, im_info
    """
    assert len(roidb) == 1, 'Single batch only'
    imgs, roidb, mvs = get_image_mv(roidb, cfg)
    im_array = imgs
    im_info = [np.array([roidb[i]['im_info']], dtype=np.float32) for i in range(len(roidb))]
    mv_array = mvs
    ...
    data = [{'data': im_array[i],
            'im_info': im_info[i],
            'motion_vector': mv_array[i]} for i in range(len(roidb))]
    label = {}
