...
class ImageNetVID(IMDB):
    ...
    def load_image_set_index(self):
        ...
        if len(lines[0]) == 2:
            # Example of one line: val/ILSVRC2015_val_00000000/000000 1
            # self.image_set_index = val/ILSVRC2015_val_00000000/000000
            self.image_set_index = [x[0] for x in lines]
            # self.frame_id = 1
            self.frame_id = [int(x[1]) for x in lines]
        else:
            # Example of one line: train/ILSVRC2015_VID_train_0000/ILSVRC2015_train_00000000 1 10 300
            # image_set_index = [train/ILSVRC2015_VID_train_0000/ILSVRC2015_train_00000000/000010, ...]
            self.image_set_index = ['%s/%06d' % (x[0], int(x[2])) for x in lines]
            # pattern = train/ILSVRC2015_VID_train_0000/ILSVRC2015_train_00000000/%06d
            self.pattern = [x[0]+'/%06d' for x in lines]
            # frame_id = 1
            self.frame_id = [int(x[1]) for x in lines]
            # frame_seg_id = 10
            self.frame_seg_id = [int(x[2]) for x in lines]
            # frame_seg_len = 300. This is the number of files in current video snippet.
    ...
    def gt_roidb(self):
        ...
        print('self.image_set_index: ', self.image_set_index)

        gt_roidb = []
        for index in range(0, len(self.image_set_index)):
          print(index)
          gt_roidb.append(self.load_vid_annotation(index))
    ...
    def load_vid_annotation(self, iindex):
        ...
        """
        # Example: index = train/ILSVRC2015_VID_train_0000/ILSVRC2015_train_00000000/000010
        ...
        if hasattr(self,'frame_seg_id'):
            # roi_rec['pattern'] = self.data_path/Data/DET/train/ILSVRC2015_VID_train_0000/ILSVRC2015_train_00000000/%06d
            roi_rec['pattern'] = self.image_path_from_index(self.pattern[iindex])
            # roi_rec['frame_seg_id'] = 10
            roi_rec['frame_seg_id'] = self.frame_seg_id[iindex]
            # roi_rec['frame_seg_len'] = 300
        ...
        # roi_rec is a region of interst record or dictionary, containing 'image',
        #   'frame_id', 'pattern', 'frame_seg_id', 'frame_seg_len', 'width', 'height',
        #   'bounding boxes', 'gt_classes', 'gt_overlaps', 'max_classes', 'max_overlaps', 'flipped'
