...
class RPNAccMetric(mx.metric.EvalMetric):
    def __init__(self):
        ...
        # self.pred = ['rpn_cls_prob', 'rpn_bbox_loss']
        # self.label = ['rpn_label', 'rpn_bbox_target', 'rpn_bbox_weight']
        self.pred, self.label = get_rpn_names()

    def update(self, labels, preds):
        # preds = [??]
        # labels: ['label', 'bbox_target', 'bbox_weight']
        # label = ['rpn_label', 'rpn_bbox_target', 'rpn_bbox_weight']

        pred = preds[self.pred.index('rpn_cls_prob')]
        # print('preds: ', preds)
        # print('RPNAccMetric, pred: ', pred)
        # print('RPNAccMetric, self.label.index(\'rpn_label\'): ', self.label.index('rpn_label'))

        # Note that rpn_label is one of -1, 0, and 1. -1 will not be used. 0 for background, 1 for non-background.
        label = labels[self.label.index('rpn_label')]
        # print('label: ', ' '.join([str(element.asnumpy()[0]) for element in label[0]]))
        # print('unique label: ', set([str(element.asnumpy()[0]) for element in label[0]]))
...
class RCNNAccMetric(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        # pred = ['rcnn_cls_prob', 'rcnn_bbox_loss']
        # label = ['rcnn_label', 'rcnn_bbox_target', 'rcnn_bbox_weight']
        # print('RCNNAccMetric, self.pred.index(\'rcnn_cls_prob\'): ', self.pred.index('rcnn_cls_prob'))
        pred = preds[self.pred.index('rcnn_cls_prob')]
        # print('RCNNAccMetric, pred: ', pred)
