...
class Module(BaseModule):
    ...
    def update_metric(self, eval_metric, labels):
        ...
        """
        # self._exec_group = DataParallelExecutorGroup(xxx)
...
class MutableModule(BaseModule):
    ...
    def bind(self, data_shapes, label_shapes=None, for_training=True,
        ...
        # print('bind(), data_shapes[0]: ', data_shapes[0])
    ...
    def fit(self, train_data, eval_data=None, eval_metric='acc',
        ...
        for epoch in range(begin_epoch, num_epoch):
            ...
            for nbatch, data_batch in enumerate(train_data):
                ...
                self.update_metric(eval_metric, data_batch.label)
                # self.backward()
                mv_out = self.get_outputs()[-1].asnumpy()
                for mv in mv_out:
                    pickle.dump(mv, open("/home/jingtun/feat_flow_compare/mv_%06d.pkl" % self.flow_count, 'wb'), protocol=2)
                    self.flow_count += 1
                self.update()
            ...
            print('fit(), one epoch has finished')
    ...
    def forward(self, data_batch, is_train=None):
        ...
        for pre, cur in zip(current_shapes, input_shapes):
            # print('forward(), pre: ', pre)
            # print('forward(), cur: ', cur)
            for k, v in pre.items():
                if k == 'motion_vector' and k not in cur:
                    continue
                if k == 'data_key' and k not in cur:
                    continue
                if k == 'data' and k not in cur:
                    continue
                if v != cur[k]:
                    # print('v: ', v, ', k: ', k, ', cur[k]: ', cur[k])
                    shape_changed = True
        ...
        if shape_changed:
            ...
            # print('forward(), second, data_batch.provide_data: ', data_batch.provide_data)
